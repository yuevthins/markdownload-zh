...
extractor.unlisted.ts 在目标页面执行
    ├── 普通站点: Readability.js 提取 → Turndown 转 Markdown
...
- `lib/sites/` — 站点适配器注册表（12 个适配器文件覆盖 68 个站点）+ `helpers.ts` 工厂函数
- `lib/extract/` — Stage 2: Readability + 后备提取（支持 `customExtract` + `needsSourceDoc`）
...
12 个适配器文件，覆盖 68 个站点。在 `lib/sites/adapters/` 下：
...
生产构建输出到 `markdownload-zh/.output/chrome-mv3/`，可直接在 Chrome 加载。`extractor.js`（~89KB）包含打包后的 Readability.js、Turndown 和所有站点适配器。
...
GitHub Issues on `yuevthins/markdownload-zh` (uses `gh` CLI). See `docs/agents/issue-tracker.md`.
